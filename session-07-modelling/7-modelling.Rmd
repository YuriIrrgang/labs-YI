...
y ~ x + I(x^2) # Adds the higher order term x^2. #if you want to use formula 
```
...
```{r, echo= FALSE}
formula_string_1 <- paste("life_expectancy", "~ gdp", "+ population", "+ alcohol")
formula_1 <- as.formula (formula_string_1)
reg_1 <- lm(formula_1, data = life_expec_dat)
reg_1

formula_string_2 <- paste(formula_string_1, "+ gdp * population")
formula_2 <- as.formula(formula_string_2)
reg_2 <- lm(formula_2, data = life_expec_dat)
reg_2

formula_string_3 <- paste("life_expectancy", "~ gdp + I(gdp^2) + log(population)")
formula_3 <- as.formula(formula_string_3)
reg_3 <- lm(formula_3, data = life_expec_dat)
reg_3
```
:::
...
```{r, echo= FALSE}
mod_table_YI <- modelsummary::modelsummary(multiv_model_out[29:31], 
                                        output = "kableExtra",
